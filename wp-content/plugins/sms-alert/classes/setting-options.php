...
class smsalert_Setting_Options {
	...
	public static function get_settings() {
					...
					<div class="SMSAlert_nav_box SMSAlert_nav_<?php echo strtolower(str_replace(' ', '_', $tab['nav'])) ; ?>_box <?php echo strtolower(str_replace(' ', '_', $tab['nav'])) ;?>">
						<div class="tabset">
							<?php foreach($tab['inner_nav'] as $in_tab){ ?>
								<input type="radio" name="tabset<?php echo $sno; ?>" id="tab<?php echo strtolower(str_replace(' ', '_', $in_tab['title'])).str_replace(' ', '_', $tab['nav']) ; ?>" aria-controls="<?php echo strtolower(str_replace(' ', '_', $in_tab['title'])).str_replace(' ', '_', $tab['nav']) ; ?>" <?php echo (!empty($in_tab['first_active']))? "checked":'';?>>
								<label for="tab<?php echo strtolower(str_replace(' ', '_', $in_tab['title'])).str_replace(' ', '_', $tab['nav']) ; ?>"><?php echo $in_tab['title'];?></label>
							<?php }?>
							...
							<div class="tab-panels">
								...
								?>
								<section id="<?php echo strtolower(str_replace(' ', '_', $in_tab['title'])).str_replace(' ', '_', $tab['nav']) ; ?>" class="tab-panel">
									<?php
		...
		<!--modal confirmation-->
			<div class="sa-modal sa-fade">
				<div id="confirm" class="sa-modal-dialog">
				<div class="sa-modal-header">
					<h5 class="sa-modal-title"><?php _e( 'Are you sure?', 'sms-alert' ); ?></h5>
					<button type="button" class="sa-close" data-dismiss="sa-modal" aria-label="Close">
					  <span aria-hidden="true">×</span>
					</button>
				  </div>
				  <div class="sa-modal-body">
					<?php _e( 'This action can not be reversed. You will be logged out of SMS Alert plugin.', 'sms-alert' ); ?>
				  </div>
				  <div class="sa-modal-footer">
					<button type="button" data-dismiss="sa-modal" class="button button-danger" id="confirmed">Yes</button>
					<button type="button" data-dismiss="sa-modal" class="button button-primary btn_cancel">No</button>
				  </div>
				</div>
			</div>
			<div class="sa-modal-backdrop sa-fade"></div>
	<!--modal confirmation-->
