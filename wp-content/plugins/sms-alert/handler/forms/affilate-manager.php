	...
	class AffiliateManagerForm extends FormInterface
	{
		...
		**/
		...
		public static function _after_changed_wpam_status($data=array())
		{
			...
			$buyer_sms_content 	= smsalert_get_option( 'wpam_sms_body_'.$status, 'smsalert_wpam_message', sprintf(__('Hello %s, status of your affiliate account %s with %s has been changed to %s.','sms-alert'), '[first_name]', '[affiliate_id]', '[store_name]', '[affiliate_status]') );
		...
		public static function getCustomerTemplates()
		{
			...
			foreach($wpam_statuses as $ks  => $vs){
				...
				$text_body 		= smsalert_get_option('wpam_sms_body_'.$vs, 'smsalert_wpam_message', sprintf(__('Hello %s, status of your affiliate account %s with %s has been changed to %s.','sms-alert'), '[first_name]', '[affiliate_id]', '[store_name]', '[affiliate_status]'));
