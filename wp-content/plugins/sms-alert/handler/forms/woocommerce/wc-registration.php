...
class WooCommerceRegistrationForm extends FormInterface
{
	...
	public static function getSignupTemplates()
	{
		...
		foreach($wc_user_roles as $role_key  => $role){
			...
			$text_body 			= smsalert_get_option('signup_sms_body_'.$role_key, 'smsalert_signup_message', sprintf(__('Hello %s, Thank you for registering with %s.','sms-alert'), '[username]', '[store_name]'));
