...
class WooCommerceCheckOutForm extends FormInterface
{
	...
	function add_custom_button()
	{
		...
		echo '$sa("#smsalert_otp_token_submit").click(function(o){ if(counterRunning){$sa("#myModal").show();return false;}';
			if($validate_before_sending_otp=='on')
			{
				...
				if($sa(".wc-terms-and-conditions #terms").prop("checked")===false)
				{
					error=error + 1;
				}
	...
	function my_custom_checkout_field( $checkout )
	{
		...
		echo ',$sa(".woocommerce-error").length>0&&$sa("html, body").animate({scrollTop:$sa("div.woocommerce").offset().top-50},1e3),$sa("#smsalert_otp_token_submit").click(function(o){var e=$sa("input[name=billing_email]").val(),';
	...
	*/
	...
	public static function trigger_after_order_place( $order_id, $old_status, $new_status ) {
		...
		if(smsalert_get_option( 'admin_notification_'.$new_status, 'smsalert_general', 'on' ) == 'on' && $admin_phone_number!='')
		{
			...
			$default_admin_sms 			= (($default_template!='') ? $default_template : sprintf(__('%s status of order %s has been changed to %s.','sms-alert'), '[store_name]:', '#[order_id]', '[order_status]'));
