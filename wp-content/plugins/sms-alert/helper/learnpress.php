...
class SmsAlertLearnPress
{
	...
	public static function getCustomerTemplates()
	{
		...
		foreach($lpress_statuses as $ks  => $vs){
			...
			$text_body 			= smsalert_get_option('lpress_sms_body_'.$vs, 'smsalert_lpress_message', sprintf(__('Hello %s, status of your %s with %s has been changed to %s.','sms-alert'), '[username]', '[order_id]', '[store_name]', '[order_status]'));
	...
	public static function getAdminTemplates()
	{
		...
		foreach($lpress_statuses as $ks  => $vs){
			...
			$text_body 			= smsalert_get_option('lpress_admin_sms_body_'.$vs, 'smsalert_lpress_message', sprintf(__('%s status of order %s has been changed to %s.','sms-alert'), '[store_name]:', '#[order_id]', '[order_status]'));
