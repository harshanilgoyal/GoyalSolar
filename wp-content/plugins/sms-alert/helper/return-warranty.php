	...
	class sa_Return_Warranty
	{
		...
		public static function getReturnWarrantyTemplates()
		{
			...
			foreach($wc_warrant_status as $ks                           => $vs){
				...
				$text_body 			                                    = smsalert_get_option('sms_text_'.$vs, 'smsalert_warranty', '') ? smsalert_get_option('sms_text_'.$vs, 'smsalert_warranty', '') : sprintf(__('Hello %s, status of your warranty request no. %s against %s with %s has been changed to %s.','sms-alert'), '[billing_first_name]', '[rma_number]', '[order_id]', '[store_name]', '[rma_status]');
