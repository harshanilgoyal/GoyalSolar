...
class SA_Abandoned_Cart{
	...
	private function load_dependencies(){
		...
		//require_once plugin_dir_path( dirname( __FILE__ ) ) . 'helper/sa-loader.php';

		//require_once plugin_dir_path( dirname( __FILE__ ) ) . 'helper/abandoned-cart-admin.php';

		//require_once plugin_dir_path( dirname( __FILE__ ) ) . 'helper/abandoned-cart-public.php';
	...
	public function addTabs($tabs=array())
	{
		...
		= self::getContentFromTemplate('template/ab-cart-setting-template.php',$smsalertcart_param);
	...
	public function getSMSAlertCartTemplates()
	{
		...
		if(empty($scheduler_data)){
			$scheduler_data['cron'][0]= array('frequency'=>'60','message'=>SmsAlertMessages::showMessage('DEFAULT_AB_CART_CUSTOMER_MESSAGE'));
		}
...
class SA_Cart_Public{
	...
	function read_cart(){
		...
		return $results_array = array('cart_total' => $cart_total, 'cart_currency' => $cart_currency, 'current_time' => $current_time, 'msg_sent' => $msg_sent, 'session_id' => $session_id, 'product_array' => $product_array);
	}
