...
class SmsAlertEMBooking
{
	...
	public static function getCustomerTemplates()
	{
		...
		foreach($embk_booking_statuses as $ks  => $vs){
			...
			$text_body 			= smsalert_get_option('embk_sms_body_'.$vs, 'smsalert_embk_message', sprintf(__('Hello %s, status of your booking %s with %s has been changed to %s.','sms-alert'), '[#_BOOKINGNAME]', '[#_BOOKINGID]', '[#_EVENTNAME]', '[#_BOOKINGSTATUS]'));
	...
	public static function getAdminTemplates()
	{
		...
		foreach($embk_booking_statuses as $ks  => $vs){
			...
			$text_body 			= smsalert_get_option('embk_admin_sms_body_'.$vs, 'smsalert_embk_message', sprintf(__('%s status of order %s has been changed to %s.','sms-alert'), '[#_BOOKINGNAME]', '[#_BOOKINGID]', '[#_EVENTNAME]', '[#_BOOKINGSTATUS]'));
