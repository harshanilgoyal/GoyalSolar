...
class SmsAlertEdd
{
	...
	public static function getCustomerTemplates()
	{
		...
		foreach($edd_order_statuses as $ks  => $vs){			
			...
			$checkboxNameId		= 'smsalert_edd_general[edd_order_status_'.$vs.']';
			$textareaNameId		= 'smsalert_edd_message[edd_sms_body_'.$vs.']';

			$text_body 			= smsalert_get_option('edd_sms_body_'.$vs, 'smsalert_edd_message', sprintf(__('EDD:Hello %s, status of your %s with %s has been changed to %s.','sms-alert'), '[first_name]', '[order_id]', '[store_name]', '[order_status]'));
	...
	public static function getAdminTemplates()
	{
		...
		foreach($edd_order_statuses as $ks  => $vs){
			...
			$checkboxNameId		= 'smsalert_edd_general[edd_admin_notification_'.$vs.']';
			$textareaNameId		= 'smsalert_edd_message[edd_admin_sms_body_'.$vs.']';

			$text_body 			= smsalert_get_option('edd_admin_sms_body_'.$vs, 'smsalert_edd_message', sprintf(__('%s status of order %s has been changed to %s.','sms-alert'), '[store_name]:', '#[order_id]', '[order_status]'));
