...
function xt_woofc_class() {
    ...
    if ( xt_woo_floating_cart()->access_manager()->can_use_premium_code__premium_only() ) {
        ...
        if(xt_woo_floating_cart()->frontend()->custom_payment_buttons_enabled__premium_only()) {
...
function xt_woofc_checkout_link() {
    ...
    $link = apply_filters_deprecated( 'xt_woo_floating_cart_checkout_link', array( $link ), '1.3.2', 'xt_woofc_checkout_link' );
...
function xt_woofc_checkout_label() {
    ...
    $label = apply_filters_deprecated( 'xt_woofc_lang_footer_checkout', array( $label ), '1.3.2', 'xt_woofc_checkout_label' );
...
function xt_woofc_item_title( &$product, &$cart_item, $cart_item_key ) {
    ...
    if ( ! $product_permalink ) {
        return wp_kses_post( apply_filters( 'woocommerce_cart_item_name', sprintf( '<span class="xt_woofc-product-title-inner">%s</span>', $product->get_name() ), $cart_item, $cart_item_key ) );
    } else {
        return wp_kses_post( apply_filters( 'woocommerce_cart_item_name', sprintf( '<a class="xt_woofc-product-title-inner" href="%s">%s</a>', esc_url( $product_permalink ), $product->get_name() ), $cart_item, $cart_item_key ) );
    }
}
 ...
 * @return string
 */
function xt_woofc_quantity_input( $args = array(), $product, $cart_item_key, $cart_item) {
