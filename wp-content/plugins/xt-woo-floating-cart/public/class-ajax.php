...
class XT_Woo_Floating_Cart_Ajax {
	...
	public function __construct(&$core) {
		...
		// Add WC Ajax Events
		$this->core->plugin_loader()->add_filter($this->core->plugin_prefix('wc_ajax_add_events'), $this, 'ajax_add_events', 1);

		// Remove WC Ajax Events
		$this->core->plugin_loader()->add_filter($this->core->plugin_prefix( 'wc_ajax_remove_events' ), $this, 'ajax_remove_events', 1);

		// Set Fragments
		$this->core->plugin_loader()->add_filter( 'woocommerce_add_to_cart_fragments', $this, 'cart_fragments', 1);
		$this->core->plugin_loader()->add_filter( 'woocommerce_update_order_review_fragments', $this, 'cart_fragments', 1 );

		// Remove / Restore hooks
		$this->core->plugin_loader()->add_filter( 'woocommerce_remove_cart_item', $this, 'remove_cart_item', 10, 2 );
		$this->core->plugin_loader()->add_filter( 'woocommerce_cart_item_restored',  $this, 'cart_item_restored', 10, 2 );

		// Added to cart action
		$this->core->plugin_loader()->add_action( 'woocommerce_add_to_cart', $this, 'added_to_cart', 10, 0 );
	...
	public function cart_fragments( $fragments ) {
		...
		if ( $this->core->access_manager()->can_use_premium_code__premium_only() ) {
            ...
            if( $frontend->coupon_list_enabled() ) {
                ...
                ob_start();
                $frontend->render_coupon_list__premium_only();
                $fragments['.xt_woofc-coupons'] = ob_get_clean();
            }

            if( $frontend->checkout_form_enabled() || $frontend->totals_enabled() ) {

			    ob_start();
                $frontend->render_totals__premium_only();
			    $fragments['.xt_woofc-body .xt_woofc-cart-totals'] = ob_get_clean();

                if (!$update_order_review) {
                    ob_start();
                    $this->core->get_template('parts/checkout/shipping-fields');
                    $fragments['.xt_woofc-body .woocommerce-shipping-fields'] = ob_get_clean();
                }
            }

			if($frontend->suggested_products_enabled() && $type !== 'update' && !$update_order_review) {

			    ob_start();
                $frontend->render_suggested_products__premium_only();
                $fragments['.xt_woofc-sp'] = ob_get_clean();
            }

			if(!$add_to_cart && !$single_add_to_cart) {
                ob_start();
                $frontend->render_custom_payment_buttons__premium_only();
                $fragments['.xt_woofc-payment-btns'] = ob_get_clean();
            }
		}
	...
	public function remove_item() {
		...
		if (!empty( $cart_item_key ) ) {
			...
			$this->set_notice( sprintf(
				...
				'<a class="xt_woofc-undo" href="#0">'.esc_html__( 'Undo', 'woo-floating-cart' ) ).'</a>'
			);
		}
