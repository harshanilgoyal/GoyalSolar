	...
	$(function() {
		...
		function initEvents() {
			...
			$(document.body).on(clickSelector, '.xt_woofc-undo', function(evt){
				...
				setCartNotice(XT_WOOFC.lang.restoring);
				showLoading(true);
		...
		function initSuggestedProductsSlider(){
			...
			suggestedProductsSlider = $('.xt_woofc-sp-products').lightSlider({
				...
				enableDrag: false,
				adaptiveHeight: true,
				controls: (XT_WOOFC.suggested_products_enabled === "1"),
			...
			if(customizer) {
				...
				if(XT_WOOFC.can_use_premium_code) {
					...
					var requireWindowResize = [
						...
						'voffset_tablet',
						'voffset_mobile',
						'cart_autoheight_enabled',
						'cart_dimensions_unit',
						'cart_width',
						'cart_height',
						'cart_width_tablet',
						'cart_height_tablet',
						...
						'cart_product_squared_thumb',
						'cart_product_thumb_width_desktop',
						'cart_product_thumb_width_tablet',
						'cart_product_thumb_width_mobile'
					];
				...
				$(top.document.body).on('mouseenter', '.customize-control-xirki-slider', function() {
					$$(cartContainer).addClass('xt_woofc-no-transitions');
				});

				$(top.document.body).on('mouseleave', '.customize-control-xirki-slider', function() {
