...
abstract class XT_Framework
{
    ...
    public function load_plugin()
    {
        ...
        $this->framework_loader()->add_action( 'wp_enqueue_scripts', $this, 'register_common_assets', 1 );
    ...
    public function register_common_assets()
    {
        ...
        wp_register_script('xt-jquery-ajaxqueue', xtfw_dir_url(XTFW_DIR_ASSETS) . '/js/jquery.ajaxqueue'.XTFW_SCRIPT_SUFFIX.'.js', array('jquery'), XTFW_VERSION, true);
	    wp_register_script('xt-jquery-touch', xtfw_dir_url(XTFW_DIR_ASSETS) . '/js/jquery.touch'.XTFW_SCRIPT_SUFFIX.'.js', array('jquery'), XTFW_VERSION, true);
        wp_register_script('xt-observers-polyfill', 'https://polyfill.io/v3/polyfill.min.js?features=ResizeObserver%2CMutationObserver', array(), XTFW_VERSION, true);
	...
	public function init_wc_ajax()
	{
		if (
			empty($this->wc_ajax) &&
			($this->plugin_loader()->has_filter($this->plugin_prefix('wc_ajax_add_events')) || $this->plugin_loader()->has_filter($this->plugin_prefix('wc_ajax_remove_events'))) &&
            function_exists('wc')
	    ) {
    ...
    public function get_account_link__premium_only($title = null, $linkClasses = array()) {
        ...
        $link = '<span><a class="'.esc_attr($linkClasses).'" href="'.esc_url($account_url).'">'.$title.'</a></span>';
